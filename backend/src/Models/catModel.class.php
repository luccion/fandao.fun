...
class CatModel
{
	...
	public static function get()
	{
		...
		is_null(self::$cats) && self::$cats = self::$db->getAll('select id,cat_name,parent_id,postable from category');
	}

	/*
		栏目添加
		fucntion add
		param: array	$data	格式: ['cat_name'=>'栏目名','intro'=>'栏目介绍','parent_id'=>'父栏目id']
		return bool
	 */
	...
	/*
		...
		function getInfo
		param: int	$id			栏目id
		param: bool	$getIntro	是否获取栏目简介
		return string/bool
	 */
	public static function getInfo($id, $getIntro = true)
	{
		self::get();
		$columns = $getIntro ? 'cat_name,intro,postable' : 'cat_name';
	...
	/*
		制作FontAwesome 数字字体
		function fontawesomeNumber
		param: int	$number
		return string	
	 */

	public static function fontawesomeNumber($number)
	{
