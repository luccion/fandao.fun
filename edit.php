...
<body>
	...
	<form method="post">
		<?php if ($f === '') { ?>
			<label>标题：<input value="<?php echo base64_decode($info['title']); ?>" style="width:250px" name="title" type="text" maxlength="30"></label><br /><br />
			<label>设置分类：
				<select name="cat">
					<?php foreach ($cats as $k => $v) { ?>
						<option value="<?php echo $v['id']; ?>">
							<?php echo $info['cat'] == $v['id'] ? ' selected' : '' ?><?php echo str_repeat('&nbsp;&nbsp;', $v['lev']), $v['cat_name']; ?></option>
					<?php } ?>
				</select>
			</label>
		<?php } ?>
		...
		<br /><br />
		<textarea class="color-background-secondary color-font-default" id="content" name="content" cols="40" rows="15"><?php echo base64_decode($info['content']); ?></textarea>
	</form>
</body>
